...
const StatCard = ({ label, value, sub, icon: Icon, color, bg, border, loading }: StatCardProps) => (
  <div className={`flex flex-col gap-3 p-4 sm:p-5 rounded-2xl border bg-[#1a1a1a] ${border}`}>
  ...
  return (
    <div className='flex flex-col gap-1.5'>
      <div className='flex items-center justify-between text-xs'>
        ...
        <span className='text-slate-300 font-semibold'>{value} / {total}</span>
      </div>
...
const PanelCard = ({ title, icon, children, className = '' }: { title: string; icon: React.ReactNode; children: React.ReactNode; className?: string }) => (
  <div className={`rounded-2xl border border-white/10 bg-[#1a1a1a] p-5 space-y-4 ${className}`}>
...
const AdminHome = () => {
  ...
  return (
    <div className='bg-[#111] min-h-screen px-4 sm:px-6 md:px-8 py-8 sm:py-10 space-y-8'>
      ...
      <div className='flex items-start justify-between gap-4'>
        <div>
          ...
          <p className='text-slate-500 text-sm max-w-lg'>
            Desde aquí puedes gestionar todo el contenido del XXXII Congreso Nacional de Estudiantes de Agronomía.
          </p>
        ...
        <div className='shrink-0 flex flex-col items-end gap-1 pt-1'>
          ...
          {loading ? (
            ...
            <>
              <p className='text-sm font-black text-slate-100'>{activePreSale.name}</p>
              <span className={`text-xs font-bold uppercase tracking-wide ${activePreSale.booking_mode ? 'text-amber-400' : 'text-slate-400'}`}>
      ...
      <PanelCard title='Inscripciones por día' icon={<Users className='w-4 h-4 text-purple-400' />}>
        {loading ? (
          <Skeleton className='h-48' />
        ) : byDateFormatted.length > 0 ? (
          <ResponsiveContainer width='100%' height={200}>
            <LineChart data={byDateFormatted} margin={{ top: 4, right: 16, left: -20, bottom: 0 }}>
              ...
              />
              <Line type='monotone' dataKey='count' name='Inscritos' stroke='#a855f7' strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#a855f7' }} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
          <p className='text-xs text-slate-600 text-center py-8'>Sin datos de inscripciones</p>
        )}
      </PanelCard>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        ...
        {/* Validaciones */}
        <PanelCard title='Estado de validaciones' icon={<UserCheck className='w-4 h-4 text-[#fbba0e]' />}>
          {loading ? (
            <div className='space-y-3'>{[1, 2].map((i) => <Skeleton key={i} className='h-8' />)}</div>
          ) : (
            <div className='space-y-3'>
              <ProgressBar label='Validados' value={p?.validated ?? 0} total={p?.total ?? 0} color='bg-green-500' />
              <ProgressBar label='Pendientes' value={p?.pending ?? 0} total={p?.total ?? 0} color='bg-yellow-500' />
              <div className='pt-2 flex items-center justify-between text-xs text-slate-500'>
                <span>Total: {p?.total ?? 0} participantes</span>
                <Link to='/participant' className='text-[#fbba0e] hover:underline font-semibold'>Ver todos</Link>
              </div>
            </div>
          )}
        </PanelCard>
        ...
        <PanelCard title='Por tipo de cuota' icon={<DollarSign className='w-4 h-4 text-emerald-400' />}>
          {loading ? (
            ...
            <div className='flex items-center gap-4'>
                ...
                <PieChart>
                  ...
                  >
                    {p!.by_quota_type.map((_, i) => (
                      <Cell key={i} fill={QUOTA_COLORS[i % QUOTA_COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{ background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 12 }}
              ...
              <div className='flex flex-col gap-2 flex-1'>
                {p!.by_quota_type.map((qt, i) => (
                  <div key={qt.quota_type} className='flex items-center justify-between text-xs'>
                    <div className='flex items-center gap-1.5'>
                      <div className='w-2 h-2 rounded-full shrink-0' style={{ background: QUOTA_COLORS[i % QUOTA_COLORS.length] }} />
                      <span className='text-slate-400'>{qt.quota_type}</span>
                    </div>
                    <span className='text-slate-200 font-semibold'>{qt.count}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Preventa activa */}
        <PanelCard title='Preventa activa' icon={<Clock className='w-4 h-4 text-teal-400' />}>
          {loading ? (
            <Skeleton className='h-40' />
          ) : activePreSale ? (
            <div className='space-y-3'>
              <div className='p-3 rounded-xl bg-teal-500/5 border border-teal-500/20'>
                <p className='text-sm font-black text-teal-400 mb-1'>{activePreSale.name}</p>
                <p className='text-xs text-slate-500'>Inicio: {new Date(activePreSale.start_date).toLocaleDateString('es-PE')}</p>
                <p className='text-xs text-slate-500'>Fin: {new Date(activePreSale.end_date).toLocaleDateString('es-PE')}</p>
              </div>
            ...
            <div className='flex flex-col items-center justify-center py-6 text-center'>
              ...
              <Link to='/pre-sale' className='text-[#fbba0e] text-xs hover:underline mt-1 font-semibold'>
                Configurar preventa
              </Link>
            </div>
          )}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        ...
        <PanelCard title='Inscritos por preventa' icon={<ChartSpline className='w-4 h-4 text-pink-400' />}>
          {loading ? (
            <Skeleton className='h-40' />
          ) : (p?.by_pre_sale ?? []).length > 0 ? (
            <ResponsiveContainer width='100%' height={160}>
              <BarChart data={p!.by_pre_sale} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                <XAxis dataKey='pre_sale' tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#64748b', fontSize: 10 }} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{ background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, fontSize: 12 }}
                  itemStyle={{ color: '#ec4899' }}
                />
                <Bar dataKey='count' name='Inscritos' fill='#ec4899' radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <p className='text-xs text-slate-600 text-center py-8'>Sin datos</p>
        ...
        {/* Top universidades */}
        <PanelCard title='Top universidades' icon={<GraduationCap className='w-4 h-4 text-cyan-400' />}>
          {loading ? (
            <div className='space-y-2'>{[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} className='h-6' />)}</div>
                ...
                <div key={u.abbreviation} className='flex items-center gap-2'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center justify-between'>
                      <span className='text-xs text-slate-300 font-semibold truncate'>{u.abbreviation}</span>
                      <span className='text-xs text-slate-400 shrink-0 ml-2'>{u.count}</span>
                    </div>
                    <div className='h-1 w-full rounded-full bg-white/5 mt-1'>
                      <div
                        className='h-1 rounded-full bg-cyan-500'
                        style={{ width: `${((u.count / (data!.top_universities[0]?.count || 1)) * 100).toFixed(1)}%` }}
                      />
                    </div>
                  </div>
                </div>
        ...
        <div className='space-y-4'>
          <PanelCard title='Códigos generales' icon={<Barcode className='w-4 h-4 text-rose-400' />}>
            {loading ? (
              ...
              <div className='space-y-3'>
                <div className='flex items-center justify-between p-3 rounded-xl bg-green-500/5 border border-green-500/20'>
                  ...
                  <span className='text-lg font-black text-green-400'>{data?.codes.available ?? 0}</span>
                </div>
                <div className='flex items-center justify-between p-3 rounded-xl bg-red-500/5 border border-red-500/20'>
                  ...
                  <span className='text-lg font-black text-red-400'>{data?.codes.used ?? 0}</span>
                </div>
                <ProgressBar label='Uso total' value={data?.codes.used ?? 0} total={data?.codes.total ?? 0} color='bg-rose-500' />
              </div>
            )}
          </PanelCard>

          <PanelCard title='Actividades por día' icon={<ActivityIcon className='w-4 h-4 text-orange-400' />}>
            {loading ? (
              ...
              <div className='space-y-2'>
                {data!.activities.by_day.map((d) => (
                  <div key={d.day} className='flex items-center justify-between text-xs'>
                    ...
                    <span className='text-slate-200 font-semibold'>{d.count} actividades</span>
                  </div>
                ))}
              </div>
            ) : (
              <p className='text-xs text-slate-600 text-center py-4'>Sin actividades programadas</p>
            )}
          </PanelCard>
        </div>
      </div>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3'>
          {modules.map((mod) => {
            ...
            return (
              ...
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${mod.bg} ${mod.border}`}>
                  <Icon className={`w-4 h-4 ${mod.color}`} />
                </div>
                <div className='flex-1'>
                  <p className='text-slate-100 font-bold text-xs mb-0.5'>{mod.label}</p>
                  <p className='text-slate-600 text-xs leading-relaxed hidden sm:block'>{mod.description}</p>
                </div>
